...
class DeliveryCalculationsTest {
    ...
    fun testDeliveryCalculations() {
        ...
        Assert.assertEquals("Incorrect number of costs", order.packages.size, deliveries.size)
    ...
    @Test
    fun testDeliveryCalculationsSinglePackage() {

        val order = Order(100,
            listOf(PackageQuote("PKG1", 50, 30, "OFR001")))
        val orderParams = OrderAndDeliveryParams(order, DeliveryParams(2, 70, 200))

        val deliveries = OrderHandler.processDeliveries(orderParams)

        Assert.assertEquals("Incorrect number of costs", order.packages.size, deliveries.size)
        Assert.assertEquals("Incorrect cost value", "PKG1 0 750 0.42", deliveries[0].toString())
    }
